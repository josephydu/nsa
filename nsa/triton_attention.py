...
class _attention(torch.autograd.Function):
    @staticmethod
    def forward(ctx, q, k, v, block_stride, block_size, causal, sm_scale, pool_num_kv_head=0, pool_kernel_size=0, pool_stride=0, pool_padding=0, select_block_count=0, fused=False):
        # B, T, H, D
        ...
        if fused:
            s = s.reshape(q.shape[0], pool_num_kv_head, -1, *s.shape[-2:]).sum(2)
            s = s.reshape(-1, *s.shape[2:])
            s = torch.nn.functional.avg_pool1d(s, pool_kernel_size, pool_stride, pool_padding, True)
            s = s.reshape(q.shape[0], pool_num_kv_head, *s.shape[-2:])  # -> B, H, T1, T2
