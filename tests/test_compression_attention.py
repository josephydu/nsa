...
q_t = q.reshape(bs, seq_len, num_q_head, head_dim).transpose(0,2,1,3)
...
k_t = k.reshape(bs, seq_len, num_kv_head, head_dim).transpose(0,2,1,3)
...
# print(q.grad.shape)
print(f'q.gra:{q.grad[0][0][:10]}')
print(f'q_ref.grad:{q_ref.grad[0][0][:10]}')
